...
REQUIRED_COLUMNS = ['cust code', 'total', 'invoice no.', 'amount', 'date', 'due date']
...
def merge_customer_ids(df_maestro, df_mapping):
    ...
    """
    df_mapping['Cust Code'] = df_mapping['Cust Code'].astype(str).str.strip()
    df_maestro['Cust Code'] = df_maestro['Cust Code'].astype(str).str.strip()

    df_merged = pd.merge(df_maestro, df_mapping[['Cust Code', 'CUSTOMER_ID']],
                         on='Cust Code', how='left')
